...
void CamuleApp::OnCoreTimer(CTimerEvent &WXUNUSED(evt))
{
	...
	// debug/sharedwatcher-current: report any EC client that stopped talking
	// to us, with our read state for its socket. Every 5 s -- the check is a
	// walk of at most a handful of sockets and logs nothing while they are
	// healthy.
	{
		static uint64 s_msPrevEcWatch = 0;
		if (theApp->ECServerHandler && (msCur - s_msPrevEcWatch) > 5000) {
			s_msPrevEcWatch = msCur;
			theApp->ECServerHandler->DebugPollEcSockets();
		}
	}
